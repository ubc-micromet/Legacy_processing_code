...
#  Merge CR1000 AND EC data TO GENERATE A PREFILTERED DATASET
#  Generate averages of all soil measureemnts replicates
#  Estimate G (soil heat flux)
#  Check the energy balance closure pre-filtering
...
Fluxes<-read.csv(paste("./EPoutputs/BurnsBog_fluxes_compiled",".csv",sep = ""),sep=",",header=TRUE,dec=".")  
CR1000<-read.csv(paste('./DATA_CR1000/output/CR1000_gapfilled_corrected',".csv",sep = ""),sep=",",header=TRUE,dec=".")
...
write.csv(input,paste('./BB_prefiltered','.csv',sep=''),row.names=FALSE)
