...
ts<-read.csv( "./Flux-tower/met_data/met_merged/met_merged.csv",sep=",",header=TRUE,dec=".")
head(ts)
tail(ts)
...
## Corrections for soil temperature
#plot original
...
bad_q <- which(DATA$DATE >= "2014-10-25" & DATA$DATE < "2014-10-27")
DATA$SOIL_TEMP_5CM[bad_q] <- NA
DATA$SOIL_TEMP_10CM[bad_q] <- NA
DATA$SOIL_TEMP_50CM[bad_q] <- NA

bad_q <- which(DATA$DATE >= "2014-10-20 18:00:00" & DATA$DATE <= "2014-10-20 22:00:00")
DATA$SOIL_TEMP_5CM[bad_q] <- NA
DATA$SOIL_TEMP_10CM[bad_q] <- NA
DATA$SOIL_TEMP_50CM[bad_q] <- NA
...
wth <- read_csv("./Flux-tower/met_data/met_ancillary/wtd/WTH_noisy_removed.csv")

## Create new column for WTH
DATA$WTH2 <- NA
DATA$WTH2_uncorr <- NA

DATA$WTH2[pmatch(wth$Date, DATA$DATE)]<-wth$ma_WTH
DATA$WTH2_uncorr[pmatch(wth$Date, DATA$DATE)]<-wth$WTH

#plot data
plot_ly(data = DATA, x = ~DATE, y = ~WTH/100, name = 'Original', type = 'scatter', mode = 'lines') %>%
  add_trace(y = ~WTH2_uncorr, name = 'Corrected WTH by ground level measurment', mode = 'lines') %>%
  add_trace(y = ~WTH2, name = 'Moving average (1day) for old sensor only period (Before 2017-11)', mode = 'lines')

# Combined PA measurements
PA_EC1 <- which(DATA$DATE <= "2016-06-16")
...
near_station <- read_csv("./Flux-tower/met_data/met_ancillary/ECCC/ECCC_met_merged.csv")
names(near_station)
...
### 2. climateATUBC dataset: Rg, ppt (http://137.82.254.122/DataExport.aspx)
ubc <- read_csv("./Flux-tower/met_data/met_ancillary/ubc_climate/climateAtUBC.csv")
...
# Linearly interpolate small gaps where ubc data was -999
DATA$RECIP <- na.approx(DATA$RECIP, na.rm = FALSE, rule = 2)
...
write.csv(DATA,paste('/Users/saraknox/Personal Content/Projects/2014-Burns Bog/Flux-tower/met_data/met_merged/met_corrected_gapfilled','.csv',sep=''),row.names=FALSE)      
