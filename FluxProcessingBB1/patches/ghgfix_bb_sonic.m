...
nameFolds(ismember(nameFolds,{'.','..','uncorrected_UTC','uncorrected_PCT','uncorrected_sonic',...
    '2015_copy','2016_copy','2015_uncorrected_sonic_copy','2016_uncorrected_sonic_copy'})) = [];

% Define time period of interest to identify folders of interest
ts = datevec(datenum(2015,3,1,0,0,0));
    ...
    for j = 1:length(namesubFolds)
        ...
        for k = 1:length(finfo) 
            ...
            if fid ~= -1
                ...
                % For cases where headers don't line up with
                line = fgets(fid);
                h2 = regexp(line,'\t','split');
                if length(h) ~= length(h2)
                    h_new = [h(1:14), {'Vin SmartFlux (V)'}, h(15:end)];
                    head_new = strjoin(h_new,'\t');
                    fprintf(oid, '%s', head_new);
                    disp([fname ' headers did not line up'])
                else
                    fprintf(oid, '%s', head);
                end
            ...
            clear header header_new line h h2 l d d2 line2
        end
    end
end
