...
nameFolds(ismember(nameFolds,{'.','..','uncorrected_UTC'})) = []; % If error, include additional subfolders!

% Define time period of interest
ts = datevec(datenum(2018,9,7,0,0,0));
te = datevec(datenum(2019,1,25,23,30,0));
...
for i = 1:length(nameFolds)
    ...
    for j = 1:length(namesubFolds)
        ...
        path_new = [pathFolder '/uncorrected_UTC/' cell2mat(nameFolds(i)) '/' cell2mat(namesubFolds(j))];
