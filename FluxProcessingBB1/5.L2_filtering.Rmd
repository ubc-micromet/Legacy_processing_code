...
input<-read.csv(paste0(dir, '/flux_data/BB_L1.csv'))
...
# Load met data to
#met <- read.csv(paste('./met_data/met_merged/','met_corrected_gapfilled.csv',sep=""),sep=",",header=TRUE,dec=".")

met <- read.csv(paste0(dir, '/met_data/met_merged/met_corrected_gapfilled.csv'),sep=",",header=TRUE,dec=".")
...
output <- cbind(met[M_start:M_end,match(c('year','jday','hour_dec','AIR_TEMP_2M','SHORTWAVE_IN'),names(met))], 
                Data_QAQC[F_start:F_end,match(c('co2_flux','u.'),names(Data_QAQC))])
...
write.table(output, file = paste('G:/.shortcut-targets-by-id/1txCh9lZ7VGCujXGvBaJCMVnuxT-65q4K/Micromet Lab/Projects/2014-Burns\ Bog/Flux-tower (1)/flux_data/REddyProc_input/for_ustar_filtering','.txt',sep=''),row.names=FALSE,sep='\t')
```
...
EddyData.F <- fLoadTXTIntoDataframe(paste0(dir, "/flux_data/REddyProc_input/for_ustar_filtering.txt"))
...
Data_QAQC$badflux = Data_QAQC$badustar | Data_QAQC$badt | Data_QAQC$badrot | Data_QAQC$badwind # add badflux flag to table (14/06/2021) - T.Satriawan
...
Ebal_numerator <- Data_QAQC$H[F_start:F_end]+Data_QAQC$LE[F_start:F_end]
...
write.csv(Data_QAQC, paste0(dir, '/flux_data/BB_L2.csv'),row.names=FALSE)
```
...
# Create output file to use in REddyProc for ustar filtering
#output <- cbind(met[M_start:M_end,match(c('year','jday','hour_dec','AIR_TEMP_2M','SHORTWAVE_IN','RH_2M', #'VPD_hPa','SOIL_TEMP_5CM','SOIL_TEMP_10CM', 'SOIL_TEMP_50CM','WTH'),names(met))], 
 #               Data_QAQC[F_start:F_end,match(c('co2_flux','ch4_flux','LE','H','u.'),names(Data_QAQC))])

# Create output file to use in REddyProc for ustar filtering **** ADDED EXTRA VARIABLES  23/07/2020
output <- cbind(met[M_start:M_end,match(c('year','jday','hour_dec','INCOMING_PAR', 'SVWC', 
                                          'AIR_TEMP_2M','SHORTWAVE_IN','RH_2M', 'VPD_hPa',
                                          'SOIL_TEMP_5CM','SOIL_TEMP_10CM',
                                          'SOIL_TEMP_50CM','WTH'),names(met))], 
                Data_QAQC[F_start:F_end,match(c('co2_flux','ch4_flux','LE','H','u.', 'L', 'wind_dir', 'wind_speed', 'u_var'),names(Data_QAQC))])

# Reorder & rename columns
#output <- output[c("year", "jday", "hour_dec","co2_flux","ch4_flux","LE","H","SHORTWAVE_IN","AIR_TEMP_2M","SOIL_TEMP_5CM","SOIL_TEMP_10CM","SOIL_TE#MP_50CM", "RH_2M","VPD_hPa","u.","WTH")]
#names_output<-c('Year','DoY','Hour','NEE','FCH4','LE','H','Rg','Tair','Tsoil5cm','Tsoil10cm', 'Tsoil50cm', 'rH','VPD','Ustar','WTH')

# Reorder & rename columns ***ADDED EXTRA VARIABLES 23/07/2020 ** ADDED EXTRA VARIABLES 05/08/2021
output <- output[c("year", "jday", "hour_dec","co2_flux","ch4_flux","LE","H",
                   "INCOMING_PAR", "SVWC", "SHORTWAVE_IN","AIR_TEMP_2M",
                   "SOIL_TEMP_5CM","SOIL_TEMP_10CM","SOIL_TEMP_50CM", 
                   "RH_2M","VPD_hPa","u.","L", "wind_dir", "wind_speed", "u_var", "WTH")]
names_output<-c('Year','DoY','Hour','NEE','FCH4','LE','H',
                'PAR', 'SWC', 'Rg','Tair','Tsoil5cm','Tsoil10cm', 'Tsoil50cm', 
                'rH','VPD','Ustar','Obukhov_L','Wind_dir', 'Wind_speed', 'u_var','WTH')


names(output)<-names_output

#Adding the units row
UNITS<-list('-','-','-','umol_m-2_s-1','umol_m-2_s-1','Wm-2','Wm-2',
            'umol_m-2_s-1', 'm3_m-3', 'Wm-2','degC',
            'degC','degC','degC','%','hPa','ms-1','M','deg','ms-1','ms-1','cm')
...
#Saving the file:
#write.table(output, file = paste('./flux_data/REddyProc_input/for_gap_filling_partitioning_test','.txt',sep=''),row.names=FALSE,sep='\t')

write.table(output, file = paste0(dir, '/flux_data/REddyProc_input/for_gap_filling_partitioning.txt'),row.names=FALSE,sep='\t')
```
