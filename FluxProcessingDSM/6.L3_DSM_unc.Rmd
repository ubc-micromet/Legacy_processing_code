...
iStart = which(tmp$Hour == 0)[1] # first record on a new day
EddyDataWithPosix.F <- tmp[iStart:nrow(tmp),]

##iStart <- which(EddyData.F$Hour == 0.5)[1] # first record on a new day
##EddyDataWithPosix.F <- fConvertTimeToPosix(EddyData.F[iStart:nrow(EddyData.F),], 'YDH',Year = 'Year',Day ='DoY',Hour = 'Hour')
##row.names(EddyDataWithPosix.F) <- 1:dim(EddyDataWithPosix.F)[1]
...
EddyProc.C$sSetLocationInfo(Lat_deg.n = 49.08875, Long_deg.n = -122.89536, TimeZone_h.n = -8)
...
essential_variables <- grep("NEE_f$|NEE_fsd$|GPP_f$|GPP_DT.*$|Reco|Reco_DT.*$|LE_f$|LE_fsd$|H_f$|H_fsd$|FCH4_uStar_f|FCH4_uStar*|FCH4_f$|FCH4_fsd$|NEE_uStar_f|NEE_U05_f|NEE_U50_f|NEE_U95_f|NEE_uStar*|NEE_U05_fsd|NEE_U50_fsd|NEE_U95_fsd",
...
outname <-paste(Rdir,'/Flux_data/',siteID,'_NEE_uncertainty.csv',sep="")
...
outname <-paste(Rdir,'/Flux_data/',siteID,'_FCH4_uncertainty.csv',sep="")
write.csv(as.matrix(results),outname, row.names = FALSE)
