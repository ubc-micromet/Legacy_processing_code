...
Rdir <- "/Users/ziyi/Library/CloudStorage/GoogleDrive-ziyi.tzuyilu@gmail.com/.shortcut-targets-by-id/1txCh9lZ7VGCujXGvBaJCMVnuxT-65q4K/Micromet Lab/Projects/2021-DSM Delta salt marsh/Flux-tower"
setwd("/Users/ziyi/Library/CloudStorage/GoogleDrive-ziyi.tzuyilu@gmail.com/.shortcut-targets-by-id/1txCh9lZ7VGCujXGvBaJCMVnuxT-65q4K/Micromet Lab/Projects/2021-DSM Delta salt marsh/FluxProcessingDSM")
...
for(i in 1:length(met.files)) {
	...
	# Water measurements before 2021-Dec-02 16:00 should be NA
	temp$COND_1_1_1[temp$yearDOY<2021336.6] <- -9999
	temp$DO_1_1_1[temp$yearDOY<2021336.6] <- -9999
	temp$DOperc_1_1_1[temp$yearDOY<2021336.6] <- -9999
	temp$pH_1_1_1[temp$yearDOY<2021336.6] <- -9999
	temp$TW_1_1_1[temp$yearDOY<2021336.6] <- -9999
	temp$WL_1_1_1[temp$yearDOY<2021336.6] <- -9999
...
met$VPD[met$VPD<0|met$VPD>30] <- NA
# Tair=50&RH=0, max_VPD=12.36kPa -> upper bound for VPD correction is set as 30 (kPa)
...
# met0 <- met #(raw data) 
# met1 <- met #(after de-outlier)
# (1) Time Shift Correction (fixed at 2021-Nov-11 09:30 PST)            
  tsc <- which(met$Timestamp<="2021-11-11 09:30")
  #met[1:tail(tsc,1)-1,3:ncol(met)] <- met[2:tail(tsc,1),3:ncol(met)] # bring data (before 2021-Nov-11 09:30am) forward by 30min
  #met[tail(tsc,1),3:ncol(met)] <- rep(-9999,ncol(met)-1)
  mcol <- match(names(met)[!names(met) %in% c('date','time','DOY','year','yearDOY','hour','Timestamp')],names(met))
  met[1:tail(tsc,1)-1,mcol] <- met[2:tail(tsc,1),mcol] # bring data (before 2021-Nov-11 09:30am) forward by 30min
  met[tail(tsc,1),mcol] <- rep(-9999,ncol(met)-7)

# (2) Installation time (Blocking off values before installation)
  # CNR4, 2021-Sep-16 17:00 PST
  pv    <- match(c("SWIN_1_1_1","SWOUT_1_1_1","LWIN_1_1_1","LWOUT_1_1_1","ALB_1_1_1","Tc_1_1_1"),names(met)) # target variables
  UNINS <- which(met$Timestamp<="2021-09-16 17:00")
  met[UNINS,pv]<- -9999
  
  # PARin/out, 2021-Sep-16 17:00 PST
  pv    <- match(c("PPFD_1_1_1","PPFDR_1_1_1"),names(met)) # target variables
  UNINS <- which(met$Timestamp<="2021-09-16 17:00")
  met[UNINS,pv]<- -9999
  
  # water probe (Manta+20), 2021-Dec-02 14:00 PST
  pv    <- match(c("TW_1_1_1","COND_1_1_1","DOperc_1_1_1","DO_1_1_1","ORP_1_1_1","WL_1_1_1"),names(met)) # target variables
  UNINS <- which(met$Timestamp<="2021-12-02 14:00")
  met[UNINS,pv]<- -9999
...
# (4) special correction
  # Soil heat flux scale correction (incorrect installation: 2021-Aug-25 ~ Sep-09  PST)
  tcb <- "2021-08-24 15:30" # begin of the period needs to be corrected
  tce <- "2021-09-09 12:30" # end of the period needs to be corrected
  trb <- "2021-09-18 15:30" # begin of reference period
  tre <- "2021-09-27 12:30" # end of reference period
  
  pc <- which(met$Timestamp>=tcb & met$Timestamp<=tce)  # data need to be corrected
  pr <- which(met$Timestamp>=trb & met$Timestamp<=tre)  # data serve as reference
  pv <- which(grepl("SHF_",names(met))==TRUE) # target variables: SHF_?_1_1
  
  # data1: to be corrected; data2: reference
  data1 <- met[pc,pv];  max1 <- max(data1); min1 <- min(data1);  n1 <- (data1-min1)/(max1-min1);    
  data2 <- met[pr,pv];  max2 <- max(data2); min2 <- min(data2);   n2 <- (data2-min2)/(max2-min2);
  ...
  # Unreasonable pH & ORP 
    # (1) 2022-Apr-23 12:00 ~ Jun-25 15:00 PST
    # (2) 2022-Jul-02 20:00 ~ Jul-27 13:30 PST
    # (3) 2022-Jul-29 10:00 ~ Aug-30 13:00 PST
    # (4) 2022-Sep-09 00:00 ~ Sep-19 12:30 PST
  pv <- match(c("pH_1_1_1","ORP_1_1_1"),names(met)) # target variables
  pd <- which(met$Timestamp>="2022-04-23 12:00" & met$Timestamp<="2022-06-25 15:00");  met[pd,pv]<- -9999
  pd <- which(met$Timestamp>="2022-07-02 20:00" & met$Timestamp<="2022-07-27 13:30");  met[pd,pv]<- -9999
  pd <- which(met$Timestamp>="2022-07-29 10:00" & met$Timestamp<="2022-08-30 13:00");  met[pd,pv]<- -9999
  pd <- which(met$Timestamp>="2022-09-09 00:00" & met$Timestamp<="2022-09-19 12:30");  met[pd,pv]<- -9999
  
  # Unreasonable WTH
  pv <- match(c("WL_1_1_1"),names(met))
  
# (5) Regular maintenance 
  # water probe
  wpv <- match(c("TW_1_1_1","pH_1_1_1","COND_1_1_1","DOperc_1_1_1",
                 "DO_1_1_1","ORP_1_1_1","WL_1_1_1"),names(met)) # target variables
  prm <- which(met$Timestamp>="2022-06-25 09:30" & met$Timestamp<="2022-06-25 10:30");  met[prm,wpv]<- -9999
  prm <- which(met$Timestamp>="2022-08-30 10:15" & met$Timestamp<="2022-08-30 12:30");  met[prm,wpv]<- -9999
  prm <- which(met$Timestamp>="2022-09-15 08:50" & met$Timestamp<="2022-09-19 13:30");  met[prm,wpv]<- -9999
  prm <- which(met$Timestamp>="2022-09-30 10:30" & met$Timestamp<="2022-09-30 11:30");  met[prm,wpv]<- -9999
  
  # NDVI & PRI (2022-Apr-26 00:00~ 2022-May-30 09:00 PST, cable issue)
  pv <- match(c("REDin_1_1_1","REDout_1_1_1","NIRin_1_1_1","NIRout_1_1_1","NDVI_1_1_1",
                "nm532in_1_1_1","nm532out_1_1_1","nm570in_1_1_1","nm570out_1_1_1","PRI_1_1_1"),names(met)) # target variables
  UNINS <- which(met$Timestamp>="2022-04-26 00:00" & met$Timestamp<="2022-05-30 09:00")
  met[UNINS,pv]<- -9999
  
# (6) Linear regression (convert relative WTH to absolute WTH, unit:m)
...
# plot(met$WL_1_1_1,ylim = c(0,3))
# a <- met$WL_1_1_1
# a[a==-9999] <- NA
# plot(a*100,ylim = c(-100,100))
# max(a,na.rm = T)
# min(a,na.rm = T)
...
EddyProc.C <- sEddyProc$new('DSM', EddyDataWithPosix.F,c("NEE","Rg","Tair","Ustar"))
